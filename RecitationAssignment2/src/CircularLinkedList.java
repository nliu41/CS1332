...
public class CircularLinkedList<T> implements LinkedListInterface<T>, Iterable<T> {
    ...
    private class CircularIterator implements Iterator<T> {
        ...
        int position; // current position (to check bounds)
        ...
        public boolean hasNext() {
            return position < size(); // is position past size???
        }

        @Override
        public T next() {
            if (!hasNext()) { // throw exception if at end of list
                throw new NoSuchElementException("iterator is now empty");
            }
            T data = current.getData(); // temp var to store data
            position++; // iterate position var
